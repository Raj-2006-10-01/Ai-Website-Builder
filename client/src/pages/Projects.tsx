...
import { ArrowBigDownDashIcon, EyeIcon, EyeOffIcon, FullscreenIcon, Laptop2Icon, Loader2Icon, MessagesSquareIcon, SaveIcon, SmartphoneIcon, TabletIcon, XIcon } from 'lucide-react'
...
const Projects = () => {
  ...
  useEffect(() => {
    if (project && !project.current_code) {
      const intervalId = setInterval(fecthProject, 10000);
  ...
  return project ? (
    <div className='flex flex-col h-screen w-full bg-gray-900 text-white'>
      ...
      <div className='flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar'>
        ...
        <div className='flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm'>
          <button disabled={isSaving} onClick={saveProject} className='max-sm:hidden bg-gray-800 hover:bg-gray-700 text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded-sm transition-colors border border-gray-700'>
            {isSaving ? <Loader2Icon className='animate-spin' size={16} /> : <SaveIcon size={16} />} Save</button>
      ...
      <div className='flex-1 flex overflow-auto'>
        ...
        <div className='flex-1 flex p-2 pl-0 overflow-auto justify-center'>
